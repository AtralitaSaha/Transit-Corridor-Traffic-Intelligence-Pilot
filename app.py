        ...
        if len(multi_corridors) > 0:
            ...
            for corr in multi_corridors:
                ...
                st.caption(
                    ...
                    f"operational intervals where the link registered deep capacity collapse while its immediate upstream "
                    f"neighbor remained entirely clear ({n_rc_total} instances verified across the pilot timeline). This "
                    f"signature confirms the delay originates within this specific segment, validating it as a true "
                    f"root-cause bottleneck rather than an absorbed downstream queue."
                )
